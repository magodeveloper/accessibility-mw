...
class PerformanceMonitor {
  ...
  constructor() {
    ...
    setInterval(() => this.pruneOldData(), 300000); // Every 5 minutes
  }
  ...
  recordAnalysis(duration: number, cacheHit: boolean): void {
    ...
    this.metrics.cacheHitRate = ((currentHitRate * (totalAnalyses - 1)) + (cacheHit ? 100 : 0)) / totalAnalyses;
  ...
  private calculateHealthScore(): void {
    ...
    // Error rate impact
    const errorRate = (this.metrics.failedRequests / Math.max(1, this.metrics.totalRequests)) * 100;
    if (errorRate > 1) score -= Math.min(30, errorRate * 3);
    
    // Response time impact
    if (this.metrics.avgResponseTime > 1000) score -= Math.min(20, (this.metrics.avgResponseTime - 1000) / 100);
    
    // Memory usage impact
    const memoryUsagePercent = (this.metrics.memoryUsage.heapUsed / this.metrics.memoryUsage.heapTotal) * 100;
    if (memoryUsagePercent > 70) score -= Math.min(25, (memoryUsagePercent - 70) * 2);
  ...
  private checkAlerts(): void {
    ...
    if (this.metrics.avgResponseTime > this.thresholds.responseTime.error) {
      this.addAlert('error', 'avgResponseTime', this.metrics.avgResponseTime, 
        this.thresholds.responseTime.error, 'Average response time is critically high');
    } else if (this.metrics.avgResponseTime > this.thresholds.responseTime.warning) {
      this.addAlert('warning', 'avgResponseTime', this.metrics.avgResponseTime, 
        this.thresholds.responseTime.warning, 'Average response time is elevated');
    }
    ...
    if (memoryPercent > this.thresholds.memoryUsage.error) {
      this.addAlert('error', 'memoryUsage', memoryPercent, this.thresholds.memoryUsage.error, 
        'Memory usage is critically high');
    } else if (memoryPercent > this.thresholds.memoryUsage.warning) {
      this.addAlert('warning', 'memoryUsage', memoryPercent, this.thresholds.memoryUsage.warning, 
        'Memory usage is elevated');
    }
    
    // Health score alerts
    if (this.metrics.healthScore < this.thresholds.healthScore.error) {
      this.addAlert('critical', 'healthScore', this.metrics.healthScore, this.thresholds.healthScore.error, 
        'System health score is critical');
    } else if (this.metrics.healthScore < this.thresholds.healthScore.warning) {
      this.addAlert('warning', 'healthScore', this.metrics.healthScore, this.thresholds.healthScore.warning, 
        'System health score is low');
    }
  }

  private addAlert(level: PerformanceAlert['level'], metric: string, value: number, threshold: number, message: string): void {
    ...
    if (!recentAlert) {
      ...
      // Log alert
      console.warn(`[PERFORMANCE ALERT] ${level.toUpperCase()}: ${message} (${value.toFixed(2)} > ${threshold})`);
    }
  }
